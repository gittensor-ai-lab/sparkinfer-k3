...
using sparkinfer::kernels::k3::k3_mla_prefill_attention_split_f32;
using sparkinfer::kernels::k3::k3_mla_prefill_attention_workspace_bytes;
...
int main() {
    ...
    float *dq,*dc,*dw,*dg,*dy,*dy_split;
    void* workspace;
    cudaMalloc(&dq,q.size()*4); cudaMalloc(&dc,c.size()*4); cudaMalloc(&dw,w.size()*4);
    cudaMalloc(&dg,g.size()*4); cudaMalloc(&dy,g.size()*4);
    cudaMalloc(&dy_split,g.size()*4);
    const size_t workspace_bytes = k3_mla_prefill_attention_workspace_bytes(T,H,L,4);
    ...
    assert(k3_mla_prefill_attention_split_f32(
        dy_split,workspace,workspace_bytes,dq,dc,dw,dg,S,T,C,H,K,L,V,4,
        1.0f/std::sqrt((float)K),0));
    assert(cudaDeviceSynchronize()==cudaSuccess);
    std::vector<float> y(g.size()), y_split(g.size());
    cudaMemcpy(y.data(),dy,y.size()*4,cudaMemcpyDeviceToHost);
    cudaMemcpy(y_split.data(),dy_split,y_split.size()*4,cudaMemcpyDeviceToHost);
    ...
    for(int t=0;t<T;++t) for(int h=0;h<H;++h) {
        ...
        for(int v=0;v<V;++v){ double ref=0; for(int p=0;p<=S+t;++p){ double latent=0; for(int l=0;l<L;++l) latent+=(double)c[(size_t)p*K+l]*w[((size_t)h*V+v)*L+l]; ref+=std::exp(score[p]-m)*latent; } ref/=z; size_t i=((size_t)t*H+h)*V+v; ref/=1.0+std::exp(-(double)g[i]); assert(std::fabs(y[i]-ref)<2e-5); assert(std::fabs(y_split[i]-ref)<2e-5); }
    }
    cudaFree(dq); cudaFree(dc); cudaFree(dw); cudaFree(dg); cudaFree(dy);
    cudaFree(dy_split); cudaFree(workspace);
