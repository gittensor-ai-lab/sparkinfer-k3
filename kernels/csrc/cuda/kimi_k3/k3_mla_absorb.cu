...
__global__ void mla_absorb_warp_kernel(float* __restrict__ out,
    ...
    const float4* qh = (const float4*)(q_proj + (size_t)h * q_stride);
...
#pragma unroll
    ...
    if (r == 0) {
        const float* pe = q_proj + (size_t)h * q_stride + qk_nope;
...
#define K3_ABSORB_LAUNCH(W)                                                          \
    k3_pdl_launch(dim3((unsigned)((warps + (W) - 1) / (W))), dim3((W) * 32), 0,       \
