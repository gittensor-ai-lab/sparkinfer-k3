...
//
// kMlaMinSliceLen is the floor on slice LENGTH — the point past which a block spends
...
//      device can hold, bounded by kMlaMinSliceLen tokens per slice.
//
...
static int k3_mla_split_count(int n_head, int n_ctx, size_t shm, int blocks_per_sm) {
    ...
    const int by_length = n_ctx / kMlaMinSliceLen;
...
void mla_decode_attn_f32(float* out, const float* q, const float* k_cache,
    ...
    if (hpb > 1 && rslots <= kMlaMaxRSlots) {
        ...
        if (sm > 0) {
            ...
            splits = std::min(std::max(splits, 1), kMlaMaxSplitsAbs);
        }
        // A band of the heads makes `groups` small, so `want` above can raise the
        // slice count past what the scratch was sized for at the base count.
        if (!k3_mla_split_scratch(n_head, splits, kv_lora, &dev)) splits = 1;
                ...
                switch (TB) {
                    ...
                    default:
                        proj_q8_0_multirow_kernel<BLOCK, ROWS_W16>
                            <<<grid, BLOCK, 0, stream>>>(y, x, (const BlockQ8_0*)W, nb, N, stride_nb);
                        break;
                }
