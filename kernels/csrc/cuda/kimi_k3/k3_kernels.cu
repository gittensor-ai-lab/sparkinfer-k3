...
// Threads for the single-block norm, sized to the work rather than fixed at 128.
...
void rms_norm_f32(float* out, const float* x, const float* w, int n, float eps,
    ...
    // K3 runs this 327 times per token per rank — 93 attn_norm and 93 ffn_norm at
    // hidden 7168, 92 routed_norm at latent 3584, and the 24+24 MLA q/kv lora norms —
    // and every one of them was <<<1, 128>>>. The block count has to stay 1 (see the
    // kernel), so the only way to raise bytes-in-flight is more threads, wider loads,
    // or both.
    //
    ...
    if (n % 4 == 0 && rms_norm_aligned16(out) && rms_norm_aligned16(x) &&
        ...
        switch (rms_norm_block_for(n4)) {
            case 1024:
                rms_norm_vec4_kernel<1024><<<1, 1024, 0, stream>>>(out, x, w, n4, n, eps);
                return;
            case 512:
                rms_norm_vec4_kernel<512><<<1, 512, 0, stream>>>(out, x, w, n4, n, eps);
                return;
            case 256:
                rms_norm_vec4_kernel<256><<<1, 256, 0, stream>>>(out, x, w, n4, n, eps);
                return;
            default:
                rms_norm_vec4_kernel<128><<<1, 128, 0, stream>>>(out, x, w, n4, n, eps);
                return;
        }
    }
    ...
    switch (rms_norm_block_for(n)) {
        case 1024:
            rms_norm_kernel<1024><<<1, 1024, 0, stream>>>(out, x, w, n, eps);
            return;
        case 512:
            rms_norm_kernel<512><<<1, 512, 0, stream>>>(out, x, w, n, eps);
            return;
        case 256:
            rms_norm_kernel<256><<<1, 256, 0, stream>>>(out, x, w, n, eps);
            return;
        default:
            rms_norm_kernel<128><<<1, 128, 0, stream>>>(out, x, w, n, eps);
            return;
    }
}
